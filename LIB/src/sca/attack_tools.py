...
def compute_variable(PLAINTEXTS, KEY, byt, variable="p_xor_k"):
    IV = np.zeros( len(PLAINTEXTS), dtype=int )
    ...
    if variable == "hw_sbox_out":
        ...
        VARIABLE_FUNC = lambda p, k : (sbox[p ^ k])&0x01

    elif variable == "zero":
        VARIABLE_FUNC = lambda p, k : zv[p ^ k]

    for trc in range(len(PLAINTEXTS)):
        IV[trc] = VARIABLE_FUNC( PLAINTEXTS[trc][byt], KEY)
    return IV
...
def compute_pge(knownkey, LOG_PROBA, num_key_bytes=16):
    pge = [0]*num_key_bytes
    for byt in range(len(pge)):
        cparefs = np.argsort(LOG_PROBA[byt])[::-1]
        pge[byt] = list(cparefs).index(knownkey[byt])
    return pge


# Wrapper to call the Histogram Enumeration Library for key-enumeration
def bruteforce(PLAINTEXTS, KEYS, LOG_PROBA, bit_bound_end=31):
    print("")
